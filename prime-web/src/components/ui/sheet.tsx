  ...
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      className={cn(
        ...
        side === 'left' && 'inset-y-0 start-0 h-full w-3/4 border-e border-border sm:max-w-sm',
        side === 'right' && 'inset-y-0 end-0 h-full w-3/4 border-s border-border sm:max-w-lg',
        sheetVariants[side],
        className
      )}
      {...props}
...
const SheetHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-col space-y-2 text-center sm:text-end', className)} {...props} />
)
